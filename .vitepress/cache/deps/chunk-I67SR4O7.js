...
} from "./chunk-G2QKPIBM.js";
import {
  compileStyles,
  solidStateFill,
  styles2String,
  userNodeOverrides
} from "./chunk-OU5JJBFG.js";
import {
  createText,
  getIconSVG
...
// node_modules/mermaid/dist/chunks/mermaid.core/chunk-Y3GBUOFX.mjs
...
var createRoundedRectPathD = __name((x2, y2, totalWidth, totalHeight, radius) => [
  "M",
  x2 + radius,
  y2,
...
var createCylinderPathD = __name((x2, y2, width, height, rx, ry) => {
  return [
    `M${x2},${y2 + ry}`,
...
var createOuterCylinderPathD = __name((x2, y2, width, height, rx, ry) => {
  return [
    `M${x2},${y2 + ry}`,
    `M${x2 + width},${y2 + ry}`,
    `a${rx},${ry} 0,0,0 ${-width},0`,
...
var createInnerCylinderPathD = __name((x2, y2, width, height, rx, ry) => {
...
var createHexagonPathD = __name((x2, y2, width, height, m2) => {
  return [
    `M${x2 + m2},${y2}`,
    `L${x2 + width - m2},${y2}`,
...
var createCylinderPathD2 = __name((x2, y2, width, height, rx, ry, outerOffset) => {
  return [
    `M${x2},${y2 + ry}`,
    `a${rx},${ry} 0,0,0 ${width},0`,
    `a${rx},${ry} 0,0,0 ${-width},0`,
...
var createOuterCylinderPathD2 = __name((x2, y2, width, height, rx, ry, outerOffset) => {
  return [
    `M${x2},${y2 + ry}`,
    `M${x2 + width},${y2 + ry}`,
    `a${rx},${ry} 0,0,0 ${-width},0`,
...
var createInnerCylinderPathD2 = __name((x2, y2, width, height, rx, ry) => {
  return [`M${x2 - width / 2},${-height / 2}`, `a${rx},${ry} 0,0,0 ${width},0`].join(" ");
}, "createInnerCylinderPathD");
...
__name(note, "note");
var createDecisionBoxPathD = __name((x2, y2, size) => {
  return [
    `M${x2 + size / 2},${y2}`,
    `L${x2 + size},${y2 - size / 2}`,
    `L${x2 + size / 2},${y2 - size}`,
...
async function squareRect2(parent, node) {
  const options = {
    ...
    ry: 0,
    classes: "",
    labelPaddingX: ((node == null ? void 0 : node.padding) || 0) * 2,
    labelPaddingY: ((node == null ? void 0 : node.padding) || 0) * 1
  };
  return drawRect(parent, node, options);
}
...
__name(text, "text");
var createCylinderPathD3 = __name((x2, y2, width, height, rx, ry) => {
  return `M${x2},${y2}
...
var createOuterCylinderPathD3 = __name((x2, y2, width, height, rx, ry) => {
  return [
    `M${x2},${y2}`,
    `M${x2 + width},${y2}`,
    `a${rx},${ry} 0,0,0 ${0},${-height}`,
...
var createInnerCylinderPathD3 = __name((x2, y2, width, height, rx, ry) => {
...
async function erBox(parent, node) {
  ...
  if (nodeStyles && node.look !== "handDrawn") {
    ...
    const strokeStyles = (_d = allStyle == null ? void 0 : allStyle.filter((e2) => {
      return e2.includes("stroke");
    })) == null ? void 0 : _d.map((s2) => `${s2}`).join("; ");
...
//# sourceMappingURL=chunk-I67SR4O7.js.map
