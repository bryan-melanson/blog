    ...
    <div class="container">
      ...
      <div class="med-column">
        ...
        <div>
          <div
            class="education"
            v-if="!isEmpty(resume.education)"
            style="margin-top: 0.5em"
          >
            <h3 style="margin-bottom: 0.5em">EDUCATION</h3>
            <div
              v-for="(educationItem, index) in resume.education"
              :key="index"
              style="position: relative"
            >
              <h3 style="margin-bottom: 0">{{ educationItem.institution }}</h3>
              <h5>
                <a :href="educationItem.website">{{ educationItem.website }}</a>
              </h5>
              ...
              <p>
                <span>{{ educationItem.area }}</span
                ><br />
                <span> {{ educationItem.studyType }}</span>
              </p>
              <div
                v-for="(course, index) in educationItem.courses"
                :key="index"
                style="position: relative"
              >
                <li>{{ course }}</li>
      ...
      <div class="med-column">
        ...
        <div v-if="!isEmpty(resume.languages)" class="languages">
          ...
          <div style="margin-top: 10px">
            <div
              v-for="(item, k) in resume.languages"
              class="item-list"
              :key="k"
            >
      ...
      <div class="large-column">
        ...
        <div v-if="!isEmpty(resume.work)" style="margin-top: 0.5em">
          ...
          <div
            v-for="(workItem, index) in resume.work"
            :key="index"
            style="position: relative"
          >
            ...
            <h5
              ...
              "
            >
              <strong>{{ workItem.company || workItem.organization }}</strong>
              <a :href="workItem.website" target="_blank">{{
                workItem.website
              }}</a>
            </h5>
            ...
            <div
              v-for="(highlight, index) in workItem.highlights"
              :key="index"
              style="position: relative"
            >
              <li>{{ highlight }}</li>
        ...
        <div v-if="!isEmpty(resume.volunteer)" style="margin-top: 0.5em">
          ...
          <div
            v-for="(volunteerItem, index) in resume.volunteer"
            :key="index"
            style="position: relative"
          >
            ...
            >
              <strong>{{
                volunteerItem.company || volunteerItem.organization
              }}</strong>
              <a :href="volunteerItem.website" target="_blank">{{
                volunteerItem.website
              }}</a>
            </h5>
            <h5>
              {{ volunteerItem.startDate }} to {{ volunteerItem.endDate }}
            </h5>
            <p>{{ volunteerItem.summary }}</p>
            <div
              v-for="(highlight, index) in volunteerItem.highlights"
              :key="index"
              style="position: relative"
            >
              <li>{{ highlight }}</li>
  ...
  async created() {
    ...
    try {
      const response = await fetch(
        "https://gist.githubusercontent.com/bryan-melanson/38b5e5494f4a9776fe6996cebb4cc1fb/raw/2b3f5a419044cc28d83605cb85c84e7bd81fbda3/resume.json",
      );
      if (response.ok) {
      ...
      console.error(
        "An error occurred while fetching JSON Resume data:",
        error,
      );
    }
  },
